...
import requests, re, json, os, requests_html
...
class bilibili_anime():
    ...
    def search_anime(self):
        ...
        play_list = re.findall('media_type(.*?)media_bangumi', resp.text, re.S)
        print(info_list[0])
        ...
        if len(info_list) == 0:
            ...
            play_url_list = re.findall('\d","url":"(.*?)","release_date"', play_list[0], re.S)
            
            print(style, region, release_time, voice_actor, introduction, '总共' + str(len(play_url_list)) + '集')

            # 操作下载
            # print(len(play_url_list))
    ...
    def Download_Anime(self, play_url_list):
        ...
        while not flv_queue.empty():
            ...
            print(down_flv)
            s = 'you-get --format=flv360 %s' % down_flv
            print(s)
...
if __name__ == '__main__':
    p = bilibili_anime()
    p.search_anime()
    # resp = p.source('https://search.bilibili.com/all?keyword=%E7%8A%AC%E5%A4%9C%E5%8F%89')
    # print(resp.text)